...
class MainActivity : AppCompatActivity() {
    ...
    private lateinit var tvDbResult: TextView
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        tvDbResult = findViewById(R.id.tvResult)
    ...
    private fun exoPlayerInit() {
        // RTSP 스트림 URL
        val uri = "rtsp://192.168.0.5:8080/h264_pcm.sdp".toUri()
    ...
    private fun loadDataFromMySQL() {
        CoroutineScope(Dispatchers.IO).launch {
            try {
                val client = OkHttpClient()
                val request = Request.Builder()
                    .url("http://10.0.2.2:3000/api/data") // Node.js 서버 URL
                    .build()
                ...
                if (response.isSuccessful) {
                    ...
                    withContext(Dispatchers.Main) {
                        tvDbResult.text = "DB 결과: $responseData"
                    }
                } else {
                    withContext(Dispatchers.Main) {
                        tvDbResult.text = "서버 오류: ${response.code}"
                    }
                }
                ...
                e.printStackTrace()
                withContext(Dispatchers.Main) {
                    tvDbResult.text = "요청 실패: ${e.message}"
                }
            }
        }
    }
